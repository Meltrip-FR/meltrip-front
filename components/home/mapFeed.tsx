...
const MapFeed = (): any => {
  ...
  const Left = (feed: FeedProps, index: number) => (
    <div
      key={index}
      ref={fadeRef}
  ...
  const Right = (feed: FeedProps, index: number) => (
    <div
      key={index}
      ref={fadeRef}
  ...
  const End = (feed: FeedProps, index: number) => (
  ...
  return feeds.map((feed: FeedProps, index: number) => {
    switch (feed.side) {
      case "LEFT":
